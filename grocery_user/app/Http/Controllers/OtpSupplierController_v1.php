   ...
   public function signin()
{
    ...
    public function postLogin(Request $request)
    {
       ...
       if(empty($request->phone || $request->email)){
        ...
        $supplier_id = $cat_id;
        $checker = DB::table('suppliers')->where('phone',$request->phone)->where('user_type',$user_type)->exists();
        ...
        if(empty($checker)){
         ...
         $password = "test@123";
                ...
                $api_token = $password;
                $_reg = DB::insert("INSERT INTO suppliers(supplier_id,phone,password,api_token,user_type) VALUES(?,?,?,?,?)", [$supplier_id,$request->phone,$passpassword,$api_token,$user_type]);
    ...
    if ($json['status']=="success") {
      if($var == 'true'){
          $hash = DB::table('suppliers')->select('id','supplier_id','phone','api_token')->where('phone','=',$request->phone)->where('user_type','=','supplier')->get();
       $user_arr=array(
           "status" => "200",
             "token" => $token,
            "otp" =>$otp,
       ...
       $this->validate($request, [
        ...
        if(empty($checker)){
         ...
         $password = "test@123";
                ...
                $passpassword = app('hash')->make($password);
                $api_token = $password;
                $_reg = DB::insert("INSERT INTO suppliers(supplier_id,email,password,api_token,user_type) VALUES(?,?,?,?,?)", [$supplier_id,$request->email,$passpassword,$api_token,$user_type]);
