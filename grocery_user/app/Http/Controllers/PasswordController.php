   ...
   public function signin()
{
    ...
    public function index(Request $request)
    {
        ...
        $user_id = $cat_id;
        $checker = DB::table('users')->where('phone',$request->phone)->exists();
     ...
     if ($json['status']=="success") {
      ...
      print_r(json_encode($user_arr));
        ...
        $cat_id = rand(1000,1000000000);
        $user_id = $cat_id;
        $checker = DB::table('users')->where('email',$request->email)->exists();
...
public function update(Request $request){
    ...
    if(!empty($password && $request->phone)){
    $passpassword = app('hash')->make($password);
    $query = User::where('phone', $request->phone)->update(['password' => $passpassword,'api_token'=>$password]);
     if(!empty($query)){
          $user_arr=array(
            "status" => "1",
    ...
    $passpassword = app('hash')->make($password);
    $query = User::where('email', $request->email)->update(['password' => $passpassword,'api_token'=> $request->password]);
     if(!empty($query)){
          $user_arr=array(
            "status" => "1",
            "message" => "successfully update password",
