...
if($quantityold > 0 ){
    ...
    $update = $con->query("update  `tbl_cart` set  `quantity`='$quantitynew' where `product_id`='$productid' and `user_id`='$userid'");
                ...
                { 
                  $j->status  = "101";
                  $cart_msg = $langs == 'en' ? "updated successfully" : "\nتمت الاضافة بنجاح ، عدد الطلبات في السلة $"; 
                  
                    $j->message = $langs == 'en' ? "Your Cart total  quantity is $quantitynew $cart_msg" : "$quantitynew   تمت الاضافة بنجاح ، عدد الطلب"; 
                }
                else{
                $j->status  = "104";
