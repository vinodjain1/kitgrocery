...
if($language == 'ar')
{
    $sub_categoryresultdata =$con->query("select * from `tbl_sub_category` where name_ar  LIKE '$keyword%'" );
...
{
    $sub_categoryresultdata =$con->query("select * from `tbl_sub_category` where name_en  LIKE '$keyword%'" );
    while($sub_category_row=mysqli_fetch_array($sub_categoryresultdata))
    {
        array_push($result,
        array(
