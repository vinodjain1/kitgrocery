 ...
 $venderresult = $con->query("select * from `vender`  where `id`='$userid' AND `account_status` ='active'");
          //$rowvender['raduis'];
            ...
            if($status == 'Yet to accept'){
                 $ssqqll="SELECT * FROM `tbl_order` INNER JOIN `tbl_order_item` ON (`tbl_order`.id = `tbl_order_item`.order_id) where `status`='Yet to accept' and `city`='".$city."' order by `tbl_order`.id DESC";
            }
            else if($status == 'Processed' || $status == 'Accepted' || $status == 'On Road' || $status == 'Paid'|| $status == 'Cash on Delivery' )
            { 
                //echo $ssqqll="SELECT * FROM `tbl_order` INNER JOIN `tbl_order_item` ON (`tbl_order`.id = `tbl_order_item`.order_id) where `vender_id`='$userid' AND `city`='$city'  AND ( `status`='Processed'  OR `status`='Accepted' OR `status`='On Road' OR `status`='Paid' OR `status`='Cash on Delivery')  order by id desc";
               $ssqqll="SELECT * FROM `tbl_order` INNER JOIN `tbl_order_item` ON (`tbl_order`.id = `tbl_order_item`.order_id) where `vender_id`='$userid' AND `city`='$city' AND ( `status`='Processed' OR `status`='Accepted' OR `status`='On Road' OR `status`='Paid' OR `status`='Cash on Delivery') order by `tbl_order`.id DESC ";
