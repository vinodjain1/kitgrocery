...
// }
    ...
    public function index(Request $request){
        ...
        for($i = 0;$i < count($tableIds);$i++)
        {
             ...
             $jsonResult[$i]["product"] = DB::table("admin_product")
              ...
              ->select('admin_product.product_name','admin_product_category.product_category_name','admin_product.featured_image',
              'admin_product.description','brand_table.brand_name')
              ->where('product_sku',$id)
              ->get();
             $jsonResult[$i]["product_sku"] = $tableIds[$i]->product_sku;
             $jsonResult[$i]["product_id"] = $tableIds[$i]->product_id;
             $jsonResult[$i]["variant_id"] = $tableIds[$i]->variant_id;
             $jsonResult[$i]["variant_sku"] = $tableIds[$i]->variant_sku;
             $jsonResult[$i]["supplier_id"] = $tableIds[$i]->supplier_id;
             $jsonResult[$i]["unit_amount"] = $tableIds[$i]->unit_amount;
             $jsonResult[$i]["unit_amount"] = $tableIds[$i]->unit_amount;
             $jsonResult[$i]["packaging_type"] = $tableIds[$i]->packaging_type;
             $jsonResult[$i]["mrp"] = $tableIds[$i]->mrp;
             $jsonResult[$i]["discount_type"] = $tableIds[$i]->discount_type;
             $jsonResult[$i]["city_id"] = $tableIds[$i]->city_id;
             $jsonResult[$i]["availability"] = $tableIds[$i]->availability;
             $jsonResult[$i]["status"] = $tableIds[$i]->status;
            
            
        }*/
         
        $results = DB::select( DB::raw("select api.product_id,api.product_sku,ap.product_name,ap.featured_image,ap.brand_id,ap.description,apc.product_category_name,
        apsc.product_sub_category_name,api.variant_id,api.variant_sku,apv.minimum_margin,apv.discount_amount variant_discount_amount,api.supplier_id,api.mrp,
...
where api.city_id = :city_id"), array(
        ...
        foreach($results as $result){
            ...
            'inventory_discount_amount'=>$result->inventory_discount_amount,
            ];
